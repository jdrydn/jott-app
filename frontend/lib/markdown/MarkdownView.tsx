...
function BlockView({
  ...
  switch (node.type) {
    ...
    case 'horizontalRule':
      return <hr className="my-3 border-t border-gray-200" />;
    case 'codeBlock': {
      const text = ((node.content ?? []) as PMInlineNode[])
        .map((c) => (c.type === 'text' ? c.text : ''))
        .join('');
      return (
        <pre className="rounded-md bg-gray-100 px-3 py-2 text-xs text-gray-800 overflow-x-auto">
          <code>{text}</code>
        </pre>
      );
    }
  ...
  return (
    <li className="flex items-start gap-2">
      <input
        ...
        checked={checked}
        readOnly
        className="mt-1 h-3.5 w-3.5 cursor-default rounded border-gray-300"
      />
